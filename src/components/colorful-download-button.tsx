...
export default function ColorfulDownloadButton({
  className = "",
  variant = "primary",
}: {
  className?: string;
  ...
  if (loading || error) {
    return (
      <button
        disabled
        className={`inline-flex items-center gap-3 px-6 py-3 rounded-lg font-semibold transition-all duration-200 disabled:opacity-50 ${className}`}
      >
